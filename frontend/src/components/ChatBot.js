...
function MiniChart({ chart }) {
  ...
  useEffect(() => {
    ...
    ctx.font = `bold ${Math.min(13, w/20)}px Plus Jakarta Sans, sans-serif`;
    ctx.textAlign = 'center';
    ctx.fillText(chart.title || '', w / 2, 18);

    if (chart.type === 'pie') {
      const data   = datasets[0]?.data || [];
      const colors = datasets[0]?.colors || ['#1a4fd6','#1a9e5c','#e53935','#f57c00','#7b1fa2','#0097a7'];
      ...
      data.forEach((val, i) => {
        ...
        if (slice > 0.2) {
          ctx.fillStyle = '#fff';
          ctx.font = `bold ${Math.min(11, w/28)}px sans-serif`;
          ctx.textAlign = 'center';
          ctx.fillText(`₹${val >= 1000 ? (val/1000).toFixed(1)+'k' : val}`, lx, ly);
        }
        start += slice;
      });
      /* legend */
      const legY = pad.top + ch + 8;
      labels.forEach((lbl, i) => {
        const lx = (i / labels.length) * w + 10;
        ctx.fillStyle = colors[i % colors.length];
        ctx.fillRect(lx, legY, 10, 10);
        ctx.fillStyle = '#4a5068';
        ctx.font = `${Math.min(10, w/32)}px sans-serif`;
        ctx.textAlign = 'left';
        ctx.fillText(lbl, lx + 13, legY + 9);
      });
      return;
    }
    ...
    for (let i = 0; i <= ticks; i++) {
      ...
      const val = maxVal - (range / ticks) * i;
      ctx.fillStyle = '#8a90b0';
      ctx.font = `${Math.min(10,w/36)}px sans-serif`;
      ctx.textAlign = 'right';
    ...
    if (chart.type === 'line') {
      datasets.forEach(ds => {
        ...
        ctx.closePath();
        ctx.fillStyle = (ds.color || '#1a4fd6') + '22';
        ctx.fill();
        /* line */
        ctx.beginPath();
        pts.forEach((p, i) => i === 0 ? ctx.moveTo(p.x, p.y) : ctx.lineTo(p.x, p.y));
        ctx.strokeStyle = ds.color || '#1a4fd6';
        ctx.lineWidth = 2.5;
        ctx.lineJoin = 'round';
        ctx.stroke();
        /* dots */
    ...
    if (datasets.length > 1) {
      ...
      datasets.forEach(ds => {
        ctx.fillStyle = ds.color || '#1a4fd6';
        ctx.fillRect(lx, pad.top + ch + 26, 10, 10);
        ctx.fillStyle = '#4a5068';
        ctx.font = `${Math.min(10,w/36)}px sans-serif`;
        ctx.textAlign = 'left';
        ctx.fillText(ds.label || '', lx + 13, pad.top + ch + 35);
  ...
  return (
    <div style={{ background:'#f8faff', borderRadius:12, padding:'10px 8px 6px', marginTop:8, border:'1px solid #e0e6f8' }}>
      <canvas ref={canvasRef} style={{ width:'100%', height: chart.type==='pie' ? 180 : 160, display:'block' }} />
    </div>
  );
}
...
export default function ChatBot() {
  ...
  const [msgs,    setMsgs]    = useState([
    { role:'assistant', content:"👋 Hello! I'm CreditBot — your AI business assistant.\n\nAsk me anything:\n• \"What's my total balance?\"\n• \"Show top debtors chart\"\n• \"Monthly trend graph\"\n• \"Add ₹500 to Rahul\"\n• \"Who owes me the most?\"", ts: new Date() }
  ]);
  ...
  const send = useCallback(async (text) => {
    ...
    try {
      ...
      if (action?.refresh) {
        setTimeout(() => {
          setMsgs(prev => [...prev, { role:'assistant', content:'🔄 Done! Refresh the page to see updates.', ts:new Date(), isHint:true }]);
        }, 600);
      }
      if (!open) setUnread(u => u + 1);
    } catch {
      setMsgs(prev => [...prev, { role:'assistant', content:'❌ Could not connect. Check your internet.', ts:new Date() }]);
    ...
    <>
      {open && (
        <div style={{
          ...
          width:360, height:560, maxHeight:'72vh',
          background:'white', borderRadius:20,
          boxShadow:'0 8px 40px rgba(26,79,214,.25)',
          display:'flex', flexDirection:'column', zIndex:400,
          border:'1.5px solid var(--border)', overflow:'hidden',
          ...(window.innerWidth <= 480 ? { left:'50%', transform:'translateX(-50%)', width:'calc(100vw - 24px)' } : {}),
        }}>
          ...
          <div style={{ background:'linear-gradient(135deg,#1a4fd6,#0e2a8a)', padding:'12px 14px', display:'flex', alignItems:'center', gap:10, flexShrink:0 }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <p style={{ fontSize:10, color:'rgba(255,255,255,.7)', margin:0 }}>Smart Business Assistant • Charts & Insights</p>
            </div>
            <button onClick={() => setOpen(false)}
              style={{ background:'rgba(255,255,255,.2)', border:'none', color:'white', width:28, height:28, borderRadius:'50%', cursor:'pointer', fontSize:16, display:'flex', alignItems:'center', justifyContent:'center' }}>×</button>
          </div>
            ...
            {loading && (
              <div style={{ display:'flex' }}>
                <div style={{ background:'var(--bg)', borderRadius:'16px 16px 16px 4px', padding:'12px 16px', display:'flex', gap:5, alignItems:'center' }}>
                  <span style={{ fontSize:12, color:'var(--text3)', marginRight:6 }}>Thinking</span>
                  {[0,1,2].map(i => (
          ...
          {/* Quick suggestions */}
          {msgs.length <= 2 && (
            <div style={{ padding:'4px 10px 4px', display:'flex', gap:5, flexWrap:'wrap', flexShrink:0 }}>
              {SUGGESTIONS.map((s, i) => (
      ...
      <button onClick={() => setOpen(o => !o)} style={{
        position:'fixed', bottom:74, right:12, width:52, height:52, borderRadius:'50%',
        background:'linear-gradient(135deg,#1a4fd6,#0e2a8a)', border:'none', color:'white',
        fontSize:22, cursor:'pointer', boxShadow:'0 4px 24px rgba(26,79,214,.45)',
