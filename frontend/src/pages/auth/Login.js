...
export default function Login() {
  ...
  const btnStyle = (disabled) => ({
    ...
    background: disabled ? 'var(--border)' : 'linear-gradient(135deg,#1a4fd6,#0e2a8a)',
    color:'white', fontSize:15, fontWeight:700, border:'none',
    cursor: disabled ? 'not-allowed' : 'pointer',
    boxShadow: disabled ? 'none' : '0 4px 20px rgba(26,79,214,.35)',
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', flexDirection:'column', background:'linear-gradient(160deg,#1a4fd6,#0e2a8a)' }}>
      ...
      <div style={{ background:'white', borderRadius:'24px 24px 0 0', padding:'32px 24px 48px' }}>
        ...
        {step === STEP.PHONE ? (
          <>
            <h2 style={{ fontSize:22, fontWeight:800, marginBottom:4 }}>Welcome 👋</h2>
              ...
              {otp.map((digit, i) => (
                <input
                  ...
                  style={{
                    ...
                    borderRadius:12,
                    background: digit ? 'var(--blue-lt)' : 'var(--input-bg)',
