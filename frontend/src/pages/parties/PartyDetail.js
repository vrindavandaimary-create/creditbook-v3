        ...
        return (
          <button key={k} onPointerDown={e => { e.preventDefault(); press(k); }}
            style={{ padding:'20px 0', fontSize:isDel?20:26, fontWeight:isDel?400:500,
              ...
              color:isDel?'#e53935':'#1a1d2e', border:'none',
              borderBottom:'1px solid #e8ecf0', borderRight:'1px solid #e8ecf0',
    ...
    <div style={{ position:'fixed', inset:0, background:'white', zIndex:500, display:'flex', flexDirection:'column', maxWidth:'var(--maxw)', margin:'0 auto' }}>
      ...
      <div style={{ padding:'0 16px', display:'flex', flexDirection:'column', gap:10 }}>
        <div style={{ background:'#f5f7fa', borderRadius:14, padding:'13px 16px', display:'flex', alignItems:'center', gap:12 }}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#aaa" strokeWidth="2" strokeLinecap="round">
            <path d="M14 3H5a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2V10"/><polyline points="17 3 21 3 21 7"/>
          </svg>
      ...
      <div style={{ padding:'10px 14px 12px', display:'flex', gap:10, borderTop:'1px solid #eee', background:'white' }}>
        ...
        <button onClick={confirm} disabled={saving || !amount}
          style={{ flex:2, padding:'14px', borderRadius:50, border:'none', background:!amount?'#d0d0d0':accent, color:'white', fontSize:16, fontWeight:800, fontFamily:'inherit', cursor:!amount?'not-allowed':'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
  ...
  return (
    <div style={{ background:'#f0f2f5', minHeight:'100vh', display:'flex', flexDirection:'column' }}>
      <div style={{ background:'white', borderBottom:'1px solid #eee', padding:'12px 16px', display:'flex', alignItems:'center', gap:12, position:'sticky', top:0, zIndex:100 }}>
        <button onClick={() => navigate(-1)} style={{ width:36, height:36, borderRadius:'50%', background:'#f5f5f5', border:'none', fontSize:20, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', color:'#444', flexShrink:0 }}>←</button>
        ...
        {party.phone && (
          <a href={`tel:${party.phone}`} style={{ width:36, height:36, borderRadius:'50%', background:'#f5f5f5', display:'flex', alignItems:'center', justifyContent:'center', color:'#555', textDecoration:'none' }}>
        ...
        {groups.length === 0 ? (
          ...
          <div key={group.day}>
            <div style={{ display:'flex', justifyContent:'center', padding:'14px 0 6px' }}>
              <span style={{ background:'#78909c', color:'white', borderRadius:20, padding:'3px 14px', fontSize:12, fontWeight:600 }}>{group.label}</span>
            </div>
            {group.txs.map(tx => {
      ...
      <div style={{ position:'fixed', bottom:'var(--nav-h)', left:'50%', transform:'translateX(-50%)', width:'100%', maxWidth:'var(--maxw)', background:'white', borderTop:'1px solid #eee', zIndex:200 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr' }}>
          <button onClick={() => setShowAddTx('got')} style={{ padding:'16px 0', background:'white', border:'none', borderRight:'1px solid #eee', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
          ...
          <button onClick={() => setShowAddTx('gave')} style={{ padding:'16px 0', background:'white', border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
      ...
      {showAddTx && <AddTxScreen party={party} type={showAddTx} onClose={() => setShowAddTx(null)} onSaved={() => { setShowAddTx(null); load(); }} navigate={navigate}/>}
