...
/* ── Clean Modern Numpad — digits only, no operators ── */
function Numpad({ value, onChange }) {
  const press = key => {
    ...
    if (value.length >= 10) return; // cap at 10 digits
    onChange(value + key);
  };

  // 3×4 grid: 1-9, then . 0 del — no operators
  const keys = [
    '1','2','3',
    '4','5','6',
    '7','8','9',
...
function AddTxScreen({ party, type, onClose, onSaved, navigate, partyId }) {
  ...
  const openDatePicker = () => dateRef.current?.showPicker?.() || dateRef.current?.click();
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'white', zIndex:500, display:'flex', flexDirection:'column', maxWidth:'var(--maxw)', margin:'0 auto' }}>
      ...
      <div style={{ padding:'28px 24px 16px', display:'flex', flexDirection:'column', alignItems:'center' }}>
        <p style={{ fontSize:56, fontWeight:700, color: amount ? '#1a1d2e' : '#d0d0d0', lineHeight:1, letterSpacing:-2, fontVariantNumeric:'tabular-nums' }}>
          ₹{amount || '0'}
        </p>
      ...
      <div style={{ padding:'0 16px', display:'flex', flexDirection:'column', gap:10 }}>
        ...
        {/* Date */}
        <div onClick={openDatePicker} style={{ background:'#f5f7fa', borderRadius:14, padding:'13px 16px', display:'flex', alignItems:'center', gap:12, cursor:'pointer' }}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#aaa" strokeWidth="2" strokeLinecap="round">
        ...
        {/* Hidden date input */}
        <input ref={dateRef} type="date" value={date} max={todayStr()}
      ...
      <div style={{ padding:'10px 14px 12px', display:'flex', gap:10, borderTop:'1px solid #eee', background:'white' }}>
        <button
          onClick={() => { onClose(); navigate(`/more/billing?party=${party._id}`); }}
      ...
      <div style={{ flex:1, overflowY:'auto', paddingBottom:160 }}>
        {groups.length === 0 ? (
          ...
          <div key={group.day}>
            ...
            {/* ── Bubbles ──
                RECEIVED (got)  → LEFT side, DOWN arrow (↓), green circle
                GIVEN    (gave) → RIGHT side, UP arrow (↑), grey circle
            */}
            {group.txs.map(tx => {
              const isGot = tx.type === 'got';
              /* received = LEFT, given = RIGHT */
              const align = isGot ? 'flex-start' : 'flex-end';
              const bubbleRadius = isGot ? '4px 16px 16px 16px' : '16px 4px 16px 16px';
              const arrowColor = isGot ? '#1a9e5c' : '#666';
              const circleBg   = isGot ? '#e8f5e9' : '#f0f0f0';

              return (
                <div key={tx._id} style={{ display:'flex', flexDirection:'column', alignItems:align, padding:'4px 14px' }}>
                  <div
                    onClick={() => setDelTxId(tx._id)}
                    style={{ background:'white', borderRadius:bubbleRadius, border:'1px solid #e4e8ec', padding:'10px 14px', maxWidth:'72%', cursor:'pointer', boxShadow:'0 1px 3px rgba(0,0,0,.06)' }}>
                    <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                      {/* Arrow icon circle */}
                      <div style={{ width:30, height:30, borderRadius:'50%', background:circleBg, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={arrowColor} strokeWidth="2.5" strokeLinecap="round">
                          {isGot
                            /* DOWN arrow for received */
